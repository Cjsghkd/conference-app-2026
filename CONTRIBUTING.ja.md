...
もし取り組みたいタスクを見つけたら、他の人と重複して作業しないようにするためIssueに ":raising_hand:"、または単独の行に `/assign` とコメントしてください。そのIssueは自動的にあなたにAssignされ、完了するとコメントに :+1: のリアクションが付きます。

すでに他の方がAssignされているIssueの場合や、あなたに別の未クローズのIssueがAssignされている場合はAssignされず、その理由をコメントでお知らせします。

`status:blocked ⛔` ラベルが付いているIssueは、まだ着手できる状態ではありません。その場合はAssignの代わりに順番待ちへの登録をコメントでお知らせし、ラベルが外れた時点で自動的にあなたにAssignします。

今年からは、より多くの方にOSS活動の機会を持っていただきたいという思いから、お一人につき同時にAssignするIssueを1つまでとしています。担当のIssueがクローズされましたら、ぜひ次のIssueにお進みください。
...
解除後もいつでも戻っていただけますので、再開できるようになりましたら、改めてIssueにコメントしてください。お時間が必要なときや、詰まってしまったときも、いつでもお気軽にIssueで運営までお知らせください。
