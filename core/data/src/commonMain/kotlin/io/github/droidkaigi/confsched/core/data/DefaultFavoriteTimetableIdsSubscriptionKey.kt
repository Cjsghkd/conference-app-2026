package io.github.droidkaigi.confsched.core.data

import dev.zacsweers.metro.AppScope
import dev.zacsweers.metro.ContributesBinding
import dev.zacsweers.metro.Inject
import io.github.droidkaigi.confsched.core.model.FavoriteTimetableIdsSubscriptionKey
import io.github.droidkaigi.confsched.core.model.SoilIds
import soil.query.buildSubscriptionKey

@Inject
@ContributesBinding(AppScope::class)
class DefaultFavoriteTimetableIdsSubscriptionKey(private val store: FavoritesStore) :
    FavoriteTimetableIdsSubscriptionKey by buildSubscriptionKey(
        id = SoilIds.favoriteTimetableIdsSubscription,
        subscribe = { store.favoriteIds() },
    )
