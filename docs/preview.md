...
- **Tests / CI** — depend on `:core:preview:impl` from a test source set (e.g. `jvmTest`). `:feature:sessions`'s `PreviewWiringTest` proves `PreviewGraph` resolves the contributed `DefaultPreviewImageResolver` from there, and maps `preview://` URLs to `DrawableResource`s.
