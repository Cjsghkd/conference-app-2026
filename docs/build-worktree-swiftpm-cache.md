...
`:app-ios-kotlin` declares a Swift Package Manager dependency through `swiftPMDependencies`, so every Gradle sync resolves the Firebase iOS SDK package graph and builds it with `xcodebuild`. The Kotlin Gradle plugin writes the results into three directories inside the working tree:

| Directory | Contents | Shared |
| --- | --- | --- |
| `.swiftpm-locks/default/swiftPMCheckout` | SwiftPM checkout of the umbrella package | yes |
| `app-ios-kotlin/build/kotlin/swiftPMCheckout` | SwiftPM checkout passed to `xcodebuild` as `-clonedSourcePackagesDirPath` | yes |
| `app-ios-kotlin/build/kotlin/swiftImportDd` | `xcodebuild` derived data, passed as `-derivedDataPath` | no |
...
Gradle regenerates the manifests from `app-ios-kotlin/build.gradle.kts`, so a working tree changes bucket when `swiftPMDependencies` changes. Re-run the script after editing it and it moves the links across.
