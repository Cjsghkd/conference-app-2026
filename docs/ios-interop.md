...
Because iOS is almost full CMP with only a native nav bar, the Swift ↔ Kotlin boundary is small. Swift Export / Swift Package Import are experimental as of 2026, so the boundary keeps a fallback (Obj-C interop / SKIE) and stays small. Within it:

- **Kotlin → Swift (Swift calls Kotlin): Swift Export.** The native Liquid Glass bar calls Kotlin APIs (`KaigiApp`, Nav3 state) through Swift Export (Kotlin 2.2.20+), which generates idiomatic Swift without Obj-C headers and preserves module/package structure.
