...
`:tools:ksp-processor` generates what would otherwise rot by omission — [NavKey serializer registration](./navigation-navkey-serializers.md), Soil key ids, preview expansion. `NavEntryProvider` / `Navigator` are deliberately hand-written. The preview image enum is generated by a Gradle task, not KSP — see [Preview image enum generation](./preview-image-enum.md).
