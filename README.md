...
`NavEntryProvider`, a single-method interface: it implements the interface, marks it
`@ContributesIntoSet`, and Metro collects every implementation into one set at compile time. The
serializer registration those entries need is generated by Kotlin Symbol Processing (KSP) the same
way. Nothing central has to be appended to, so adding a screen adds files rather than editing
them. Features cannot reach one another either — cross-feature isolation is a Gradle module
