...
explicit — and both are compile errors. Every checker is covered by the Kotlin compiler test
framework, so the rules themselves are tested like any other code. Each rule, with the code it
rejects and why, is in [Enforcement](./docs/enforcement.md).
...
those changes apart — off each other's lines, and small enough that a person can still review
them.

**Features meet through interfaces, not shared files.** A feature reaches the app shell through
`NavEntryProvider`, a single-method interface, and marks its implementation
`@ContributesIntoSet`; Metro collects every implementation into one set at compile time, and the
shell iterates that set without naming a single feature. The serializer registration those entries
need is generated by Kotlin Symbol Processing (KSP) the same way. Nothing central has to be
appended to, so adding a screen adds files rather than editing them. Features cannot reach one
another either — cross-feature isolation is a Gradle module boundary rather than a convention. See
...
Two edits to two components end up as edits to two files, and a reviewer reads two small diffs
instead of one large one.

`scripts/new-screen.sh` writes the whole file set a new screen needs, and the generated code
compiles on all four targets and passes every checker. See
