...
one single-method interface, and the shell never names the feature back:
...
(KSP) generates the serializer registration those entries need the same way. Nothing central has
to be appended to, so adding a screen adds files rather than editing them. Features cannot reach
one another either — cross-feature isolation is a Gradle module boundary rather than a convention.
